...
# Step 4 - Transform the Unix time format to a timestamp
# Add additional columns for year, month, and day to the DataFrame

display(df_earth_quake_final)
...
df_earth_quake_expanded = df_earth_quake
# First check if ther eis any struct or array data type in columns
complex_fields = {field.name : field.dataType
    for field in df_earth_quake.schema.fields
    #print(f"{field.name}  type is : {field.dataType}")
    if isinstance(field.dataType,(StructType,ArrayType))}

# Unpack all the array or struct data types
for key , value in complex_fields.items():
    if isinstance(value,ArrayType):
        df.withColumn(key,explode_outer(col(key)))
        print(key)
        print("array")
    elif isinstance(value,StructType):
        expanded = [col(f"{key}.{field.name}").alias(f"{key}_{field.name}")
        ...
        print(expanded)
        #print(10000)
        df_earth_quake_expanded = df_earth_quake_expanded.select("*",*expanded).drop(key)
        #display(df_earth_quake_final)
        #print(key)
        #print("struct")
    #print(value)
...
display(df_earth_quake_expanded)
...
for key,value in complex_fields.items():
    ...
    if isinstance(complex_fields[key],StructType):
        print(1)
    else:
        print(2)
        print (key)

list(complex_fields.keys())[0]
...
df_earth_quake_final = spark.read.option("multiline", "true").schema(schema_quake).json(file_path)
