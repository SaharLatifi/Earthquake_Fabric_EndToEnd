...
eq_osoyoos_df = spark.read.table("earthquake_events_gold").filter(
...
if eq_osoyoos_df.count() != 0:
    start_date_replace_hyphen = start_date.replace("-","_")
    tabel_name = f"Osoyoos_events_{start_date_replace_hyphen}"
    print(tabel_name)
    eq_osoyoos_df.write.mode("append").saveAsTable(tabel_name)
